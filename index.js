...
// ============================================================
// 1. إعداد السيرفر الوهمي (Render Port)
// ============================================================
const app = express();
const PORT = process.env.PORT || 3000;

app.get('/', (req, res) => { res.send('✅ Bot is Running with Docker!'); });
app.listen(PORT, () => { console.log(`✅ Server running on port ${PORT}`); });

// ============================================================
// 2. المتغيرات (تأكد من إضافتها في Render Environment Variables)
// ============================================================
...
mongoose.connect(MONGO_URI)
    .then(() => { console.log('✅ MongoDB Connected!'); restoreSessions(); })
...
async function startUserSession(userId, ctx) {
    if (sessions[userId]) {
        if (sessions[userId].status === 'READY') {
            if (ctx) ctx.reply('✅ **متصل.**', Markup.inlineKeyboard([[Markup.button.callback('📂 خدمات', 'services_menu')], [Markup.button.callback('❌ خروج', 'logout')]]));
            return;
        }
    ...
    const client = new Client({
        ...
        puppeteer: { 
            headless: true,
            // هذا السطر هو الحل السحري لـ Docker
    ...
    try { 
        ...
        if(ctx) ctx.reply('خطأ في التشغيل. حاول مجدداً.', Markup.inlineKeyboard([[Markup.button.callback('🔄 تحديث', 'retry_login')]]));
    }
}
...
async function showMainMenu(ctx) {
    ...
    if (isAdmin || isPaid) {
        msg += isAdmin ? "👑 **المدير**\n" : `✅ **مشترك**\n`;
        buttons.push([Markup.button.callback('🔗 واتساب', 'open_dashboard')]);
        ...
        msg += `⛔ **غير مفعل**\nتواصل مع: @${adminSet ? adminSet.value : ADMIN_USERNAME_CACHE}`;
        buttons.push([Markup.button.callback('🛒 اشتراك', 'req_sub')]);
    }
...
bot.action(/manual_days_(.+)/, (ctx) => { userStates[ADMIN_ID] = { step: 'TYPE_DAYS_FOR_REQ', targetId: ctx.match[1] }; ctx.reply('الايام:'); });
...
async function sendGroupMenu(ctx, userId) {
    ...
    btns.push([Markup.button.callback('الكل', 'sel_all'), Markup.button.callback('لا شيء', 'desel_all')]);
